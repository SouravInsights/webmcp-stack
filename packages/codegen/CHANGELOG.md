...
- acd1dff: Embed the agent skill and the journey helper in the build instead of reading them from the package at runtime.
...
- 9509b9e: Add a `dev` subpath with the dashboard's shared pieces, and let the dashboard run on a host other than the dev server.
