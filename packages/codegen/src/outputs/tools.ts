...
/**
 * The pre-0.9 end marker, written with box-drawing characters. Reads still
 * accept it so files generated by an older version migrate to the plain
 * marker on the next run instead of looking like a hand edit. Written with
 * escapes so this source file stays plain text.
 */
const GENERATED_END_LEGACY =
  "// \u2500\u2500\u2500 webmcp-codegen: end generated. Your code below survives regeneration. \u2500\u2500\u2500";
...
function toolFile(
  ...
  const endMarker = existing.includes(GENERATED_END)
    ? GENERATED_END
    : existing.includes(GENERATED_END_LEGACY)
      ? GENERATED_END_LEGACY
      : undefined;
  ...
  const preservedTail = existing.slice(existing.indexOf(endMarker) + endMarker.length);
